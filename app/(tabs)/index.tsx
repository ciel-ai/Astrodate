...
import { fetchAllRegisteredUsers, fetchFinalMatches, fetchPersonalityMatches, getDiscoveryPreferences, hasCompletedOnboarding, type DiscoveryPreferences, type FinalMatchResult } from '@/lib/matching';
...
function SparklingHeart({ id, delay }: { id: number; delay: number }) {
  ...
  return (
    <Animated.View style={[styles.sparklingHeart, heartAnimatedStyle]}>
...
export default function DiscoverScreen() {
    ...
    (async () => {
      try {
        ...
        if (photosResult.success && photosResult.data) {
          ...
          if (primary?.photo_url) setCurrentUserPhoto({ uri: primary.photo_url });
        }
    ...
    (async () => {
      try {
        ...
        if (sessionData?.session?.user?.id) {
          ...
          if (astroData) setCurrentUserAstro(astroData);
        }
  ...
  const loadDiscoverProfiles = useCallback(async () => {
    try {
      ...
      const [registeredUsers, highQualityMatches, section1Response, discoveryPrefs, reportedResult] = await Promise.all([
      ...
      const filteredRegisteredUsers = registeredUsers
      ...
      // Fallback: if strict filtering hides everything, keep discovery populated with
      // users that at least satisfy age/location preferences.
      const baseCandidates = filteredRegisteredUsers.length > 0
      ...
      // Fetch personality scores for all matching target profiles in parallel
      const personalityMatchesResults = await Promise.all(
        baseCandidates.map((u: FinalMatchResult) => fetchPersonalityMatches(u.match_user_id))
      );

      const personalityMatches = personalityMatchesResults.filter(Boolean);

      // Merge results: Use highQualityMatches for users that have them (for real scores),
      // fallback to registeredUsers for others that the matching RPC might have filtered out
      const matchMap = new Map(baseHighQualityMatches.map((m: FinalMatchResult) => [m.match_user_id, m]));

      // Edge function returns array of distinct targets now
      const pMatchesArray: any[] = personalityMatches || [];

      // Build map carefully examining every item to extract ID and score
      const personalityMap = new Map();
      for (const m of pMatchesArray) {
        if (!m || typeof m !== 'object') continue;

        // Using target_user_id that the function natively returns
        const id = m.target_user_id || m.match_user_id || m.matched_user_id || m.user_id || m.id;

        // Using compatibility_score natively
        const rawScore = m.compatibility_score ?? m.personality_score ?? m.score ?? m.compatibility;

        if (id && rawScore !== undefined) {
          const finalScore = rawScore <= 1 ? rawScore * 100 : rawScore;
          personalityMap.set(id, { personality_score: Math.round(finalScore) });
        }
      }

      const mergedResults = baseCandidates.map(u => {
        const matchData = matchMap.get(u.match_user_id);
        const persData = personalityMap.get(u.match_user_id);

        let result: any = matchData ? { ...matchData } : { ...u };

        if (persData && persData.personality_score != null) {
          result.personality_score = persData.personality_score;
        }

        // --- ASTROLOGY MATCHING BLOCK ---
        // Resolve accurate signs
        const myW = currentUserAstro?.western_sign?.toLowerCase()?.trim();
        const myI = currentUserAstro?.indian_sign?.toLowerCase()?.trim();
        const theirW = (u as any)?.western_sign?.toLowerCase()?.trim();
        const theirI = (u as any)?.indian_sign?.toLowerCase()?.trim();

        // Map Western
        if (myW && theirW && astroTables.western.length > 0) {
          const row = astroTables.western.find(r => {
            const vals = Object.values(r).map(v => typeof v === 'string' ? v.toLowerCase().trim() : '');
            return vals.includes(myW) && vals.includes(theirW);
          });
          if (row) {
            const scoreKeys = Object.keys(row).filter(k => k.includes('score') || k.includes('percentage') || k.includes('compatibility'));
            const reportKeys = Object.keys(row).filter(k => k.includes('report') || k.includes('description') || k.includes('text') || k.includes('detail'));

            if (scoreKeys.length) result.western_score = Number(row[scoreKeys[0]]) || result.western_score;
            if (reportKeys.length) result.western_report = row[reportKeys[0]] || result.western_report;
          }
        }

        // Map Indian
        if (myI && theirI && astroTables.indian.length > 0) {
          const row = astroTables.indian.find(r => {
            const vals = Object.values(r).map(v => typeof v === 'string' ? v.toLowerCase().trim() : '');
            return vals.includes(myI) && vals.includes(theirI);
          });
          if (row) {
            const scoreKeys = Object.keys(row).filter(k => k.includes('score') || k.includes('points') || k.includes('ashtakoota') || k.includes('compatibility'));
            const reportKeys = Object.keys(row).filter(k => k.includes('report') || k.includes('recommendation') || k.includes('description'));

            if (scoreKeys.length) result.indian_score = Number(row[scoreKeys[0]]) || result.indian_score;
            if (reportKeys.length) result.indian_recommendation = row[reportKeys[0]] || result.indian_recommendation;
          }
        }
        // --- END ASTROLOGY MATCHING ---

        return result;
      });

      const nonReportedMergedResults = mergedResults.filter(
        (u) => !reportedSet.has(String((u as any)?.match_user_id || ''))
      );

      if (nonReportedMergedResults.length > 0) {
        const mapped = await mapMatchesToProfiles(nonReportedMergedResults);
        setProfiles(mapped);
      } else {
        setProfiles([]);
      }
  ...
  }, [astroTables.indian, astroTables.western, currentUserAstro, mapMatchesToProfiles]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const existingChannel of supabase.getChannels()) {
        if (existingChannel.topic.includes('new-onboarding-completions')) {
      ...
      channel = supabase
        ...
        .on(
          ...
          async (payload) => {
            ...
            if (!isComplete) {
              console.log(`⏭️ User ${newPhotoUserId} hasn't completed onboarding yet`);
              return;
            }
            ...
            try {
              ...
              if (newUserMatch && mounted) {
                if (!candidateMatchesInterest(newUserMatch, currentUserInterest)) {
                  console.log(`⏭️ Skipping new user ${newPhotoUserId} because they do not match current user preferences`);
                  return;
                }

                console.log(`✨ Adding new user ${newPhotoUserId} to profiles list`);
                ...
                if (newProfile.length > 0 && mounted) {
                  setProfiles((current) => {
                    ...
                    if (existingProfile) {
                      console.log(`✅ User ${newPhotoUserId} already in profiles list`);
                      return current;
                    }
  ...
  const checkAndShowMatch = useCallback(async (likedUserId: string, profile: Profile) => {
    try {
      ...
      if (result.isMatch) {
        ...
        // Channel ID is now saved in the user_matches table
        // You can use result.channelId for chat/messaging functionality
      }
    } catch (error) {
  ...
  const handleLike = useCallback(async () => {
    ...
    if (likedUserId) {
      try {
        console.log('✅ Like action triggered for user:', likedUserId);
  ...
  const handleDislike = useCallback(async () => {
    ...
    if (likedUserId) {
      try {
        console.log('❌ Dislike action triggered for user:', likedUserId);
  ...
  const handleSuperLike = useCallback(async () => {
    ...
    if (likedUserId) {
      try {
        console.log('⭐ Superlike action triggered for user:', likedUserId);
          ...
          <View style={styles.cardWrapper}>
            ...
            >
              <View style={styles.profileCard}>
                ...
                <LinearGradient
                  colors={['transparent', 'rgba(12,12,12,0.8)', '#000000']}
                  locations={[0, 0.6, 1]}
              ...
              <View key={`${String(profile.id)}-${isTop ? 'top' : 'next'}`} style={isTop ? styles.currentProfileWrapper : styles.nextProfileWrapper}>
                {isTop ? (
                  <GestureDetector gesture={composedGesture}>
                    <Animated.View style={[swipeAnimatedStyle, frontAnimatedStyle]}>
          ...
          <View style={styles.actionIconsContainer}>
            ...
            <TouchableOpacity activeOpacity={0.7} style={styles.actionButtonSmallWrapper} onPress={handleSuperLike} disabled={isTransitioning}>
              <Animated.View style={[styles.actionIconGlassy, superLikeButtonScale, superLikeButtonColorStyle]}>
                <BlurView intensity={25} tint="dark" style={StyleSheet.absoluteFill} />
                <AnimatedIonicons name="paper-plane" size={28} style={superLikeIconStyle} />
              </Animated.View>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  overlayFull: {
    ...
    right: 0,
    height: '60%',
    paddingBottom: 0, // Absolute floor of the card edge
  ...
  actionIconsFixed: {
    ...
    left: 0,
    right: 0,
    bottom: 130, // Moved up to sit above the name
  ...
  profileStackWrapper: {
    ...StyleSheet.absoluteFillObject,
    justifyContent: 'flex-end',
    paddingBottom: 0, // Absolute bottom of the screen
  },
  nextProfileWrapper: {
    position: 'absolute',
    top: 0,
