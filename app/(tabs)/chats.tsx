...
export default function ChatsScreen() {
  ...
  useEffect(() => {
    ...
    // Subscribe to all online status changes and filter client-side.
    // NOTE: Supabase postgres_changes only supports `column=eq.value` filters —
    // the `user_id=in.(...)` syntax is NOT supported and causes a post-subscribe
    ...
    const onlineChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Client-side filter: only update users we actually have chats with
  ...
  const handleUnarchive = async () => {
    ...
    const updatedChats = chats.map((chat) =>
      chat.id === selectedChat.id ? { ...chat, isArchived: false } : chat
    );
    setChats(updatedChats);
    setFilteredChats(updatedChats);
    setShowOptionsModal(false);
                  ...
                  <View style={styles.optionsList}>
                    ...
                    {selectedChat?.isArchived ? (
                      <TouchableOpacity
                        ...
                        activeOpacity={0.7}>
                        <MaterialIcons name="unarchive" size={18} color="#FFFFFF" />
