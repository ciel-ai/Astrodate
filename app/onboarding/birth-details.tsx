import { getAstroDetails } from '@/lib/astro';
...
export default function BirthDetailsScreen() {
  ...
  const geocodePlaceOfBirth = async (placeName: string) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // Calculate timezone from longitude (faster, no API call needed)
        const timezone = calculateTimezoneFromLongitude(longitude);
  ...
  const fetchAstroDetails = async () => {
    ...
    const timezoneHours = getTimezoneOffsetHours(birthDateTime, placeCoordinates?.longitude);
