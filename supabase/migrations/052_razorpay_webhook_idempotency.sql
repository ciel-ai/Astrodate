-- Razorpay webhook idempotency and atomic subscription activation.
-- This migration is intentionally non-destructive: it adds a processing ledger,
-- lookup indexes, and a service-role RPC used by the webhook Edge Function.

CREATE TABLE IF NOT EXISTS public.processed_razorpay_webhooks (
...
ALTER TABLE public.processed_razorpay_webhooks ENABLE ROW LEVEL SECURITY;

CREATE UNIQUE INDEX IF NOT EXISTS idx_processed_razorpay_webhooks_payment_id_unique
  ON public.processed_razorpay_webhooks(payment_id)
  WHERE payment_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_processed_razorpay_webhooks_order_id_unique
  ON public.processed_razorpay_webhooks(order_id)
  WHERE order_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_processed_razorpay_webhooks_payment_link_id
  ON public.processed_razorpay_webhooks(payment_link_id)
  WHERE payment_link_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_user_subscriptions_payment_link
