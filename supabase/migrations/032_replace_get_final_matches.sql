...
BEGIN
  RETURN QUERY
  WITH candidates AS (
    -- All users except self, excluding already-acted-on users
    SELECT
  ...
  scored AS (
    SELECT
      ...
      -- indian_score: reuse nakshatra portion from astro, weighted separately
  ...
  SELECT
    ...
    s.age,
    s.location,
    -- final_match_score out of 100
    ROUND(s.western_score + s.personality_score + s.indian_score, 2) AS final_match_score,
    s.personality_score,
    s.indian_score,
    s.western_score,
    -- Qualitative label for indian_score
    CASE
    ...
    -- Qualitative label for western_score
    CASE
