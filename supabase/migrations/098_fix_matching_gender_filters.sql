-- Migration 098: Fix matching gender preference filtering in get_final_matches and get_fallback_feed.
--
-- Resolves user and candidate gender preference using their section1_qns.interest (onboarding answers)
-- if user_preferences.gender_preference is null/missing.
-- Also updates get_fallback_feed to perform identical bidirectional gender filtering.

DROP FUNCTION IF EXISTS public.get_fallback_feed(uuid);
DROP FUNCTION IF EXISTS public.get_final_matches(uuid);

-- 1. Redefine get_fallback_feed with dynamic bidirectional gender filtering
...
DECLARE
  ...
  v_location          TEXT;
BEGIN
  -- Fetch viewer's preferences + their own gender
  SELECT
  ...
  -- Resolve v_gender_pref using section1_qns.interest if it is NULL
  ...
  WHERE up.user_id <> input_user_id
    ...
    -- Viewer's filter: viewer must want the candidate's gender
    AND (
      v_gender_pref IS NULL
      OR lower(v_gender_pref) IN ('everyone', 'all', '')
      OR lower(up.gender) = lower(v_gender_pref)
    )
    -- Candidate's filter: candidate must also want the viewer's gender
    AND (
      lower(COALESCE(
...
-- 2. Redefine get_final_matches with dynamic bidirectional gender filtering
...
DECLARE
  ...
  v_max_age           INT;
  v_gender_pref       TEXT;
  v_viewer_gender     TEXT;   -- viewer's own gender for reverse check
  v_location          TEXT;
...
BEGIN
  ...
  -- Step 2: Fetch viewer's preferences + their own gender
  SELECT
    COALESCE(up.min_age, 18),
    COALESCE(up.max_age, 65),
    up.gender_preference,
  ...
  INTO v_min_age, v_max_age, v_gender_pref, v_viewer_gender,
       v_location, v_preferred_elems, v_blocked_signs
  FROM public.user_profiles prof
  LEFT JOIN public.user_preferences up ON up.user_id = prof.user_id
  WHERE prof.user_id = input_user_id;
  ...
  candidates AS (
    ...
    WHERE
      ...
      -- preferred_elements: only show candidates matching preferred elements
      AND (
  ...
  scored AS (
    SELECT
      ...
      ROUND(
        CASE
          WHEN cp.user_id IS NULL THEN 0.5
          ELSE (
            (
              CASE WHEN v_introvert_ext IS NULL OR cp.introvert_extrovert IS NULL THEN 0
              ...
              CASE WHEN array_length(v_hobbies,1) > 0 AND array_length(cp.hobbies,1) > 0
                   ...
                   ELSE 0 END
              +
              CASE WHEN v_looking_for IS NULL OR cp.looking_for IS NULL THEN 0
                   WHEN v_looking_for = cp.looking_for THEN 0.9
                   ELSE 0.4 END
              +
              CASE WHEN v_planning_style IS NULL OR cp.planning_style IS NULL THEN 0
                   WHEN v_planning_style = cp.planning_style THEN 0.7
                   ELSE 0.5 END
              +
              CASE WHEN v_show_care IS NULL OR cp.show_care IS NULL THEN 0
  ...
  SELECT
    ...
    s.location,
    ROUND(
      ((s.western_score + COALESCE(s.personality_score, 15.0) + s.indian_score) * 0.85)
