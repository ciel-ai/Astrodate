-- Migration 093: Fix bidirectional gender preference filtering in get_final_matches.
--
-- Bug: candidates CTE only checked the viewer's gender_preference against
--      the candidate's gender. It never checked whether the candidate is also
--      interested in the viewer's gender. Result: a male user interested in
--      females saw all female profiles, including females only interested in
--      other females.
--
-- Fix: fetch the viewer's own gender (v_viewer_gender), LEFT JOIN
--      user_preferences for each candidate, and add a second filter that
--      requires the candidate's gender_preference to include the viewer's gender.
