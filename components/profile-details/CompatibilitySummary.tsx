...
interface CompatibilitySummaryProps {
  indianScore?: number;
  westernScore?: number;
  /** Fallback western score from zodiac_compatibility API when match has no western_score */
  zodiacScore?: number;
  finalScore?: number;
  personalityScore?: number;
  westernReport?: string;
}
...
const CompatibilitySummary = memo(function CompatibilitySummary({
  ...
  const overallPct = toPercentage(finalScore ?? indianScore ?? westernScore ?? zodiacScore);
  const indianPct = toPercentage(indianScore);
  // Use match western_score first; fall back to live zodiac compat percentage
  const westernPct = toPercentage(westernScore ?? zodiacScore);
      ...
      <View style={styles.scoresContainer}>
        ...
        {westernPct !== null && (
          <>
            ...
            {westernReport ? (
              <Text style={styles.reportText}>{westernReport}</Text>
            ) : null}
          </>
        )}
        {personalityPct !== null && (
